...
Particle::Particle(std::array<double, 3> x_arg, std::array<double, 3> v_arg, double m_arg, double gGrav_arg,
                   double sigma_arg, double epsilon_arg, double membrane_arg, int type_arg) : x(x_arg), v(v_arg),
                                                                                              m(m_arg),
                                                                                              gGrav(gGrav_arg),
                                                                                              sigma(sigma_arg),
                                                                                              epsilon(epsilon_arg),
                                                                                              membrane(membrane_arg),
                                                                                              type(type_arg) {
    f = {0., 0., 0.};
    old_f = {0., 0., 0.};
    // spdlog::debug("Particle generated!");
...
Particle::Particle(std::array<double, 3> x_arg, std::array<double, 3> v_arg, std::array<double, 3> f_arg,
                   ...
                   double membrane_arg, int type_arg) : x(x_arg), v(v_arg), f(f_arg), old_f(oldf_arg), m(m_arg),
                                                        gGrav(gGrav_arg), sigma(sigma_arg), epsilon(epsilon_arg),
                                                        membrane(membrane_arg), type(type_arg) {
}
...
//0.0 means false and 1.0 means true
bool Particle::isMembrane() {
    return membrane == 1.0;
}
...
std::string Particle::toString() const {
    ...
    stream << "Particle: X:" << x << " v: " << v << " f: " << f
           ...
           << " membrane: " << (membrane == 1.0) << " type: " << type;
    return stream.str();
}
