...
/** Note: all constructors initialize particles with membrane = false.
 * Use setMembrane method to set membrane = true. */
...
Particle::Particle(std::array<double, 3> x_arg, std::array<double, 3> v_arg, std::array<double, 3> f_arg,
                   ...
                   int type_arg) : x(x_arg), v(v_arg), f(f_arg), old_f(oldf_arg), m(m_arg), gGrav(gGrav_arg),
                                   sigma(sigma_arg), epsilon(epsilon_arg), type(type_arg), membrane(false) {
}
